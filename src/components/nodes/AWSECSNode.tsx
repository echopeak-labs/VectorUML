...
export const AWSECSNode = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'ECS Service',
    clusterName: (data as any).clusterName || '',
    serviceName: (data as any).serviceName || '',
    taskDefinition: (data as any).taskDefinition || '',
      ...
      <div className="border-t border-border px-3 py-2 space-y-2">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            onChange={(e) => updateData({ clusterName: e.target.value })}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">Task Definition</label>
          <input
            type="text"
            value={localData.taskDefinition}
            onChange={(e) => updateData({ taskDefinition: e.target.value })}
            className="bg-transparent text-xs outline-none border-b border-border focus:border-primary"
...
AWSECSNode.displayName = 'AWSECSNode';
