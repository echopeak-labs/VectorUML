...
export const AWSEC2Node = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'EC2 Instance',
    ec2Name: (data as any).ec2Name || '',
    targetGroupName: (data as any).targetGroupName || '',
    ...
    <div className={`rounded-lg border-2 bg-card shadow-lg min-w-[240px]`} style={{ borderColor: `hsl(var(--${color}))` }}>
      ...
      <div className="border-t border-border px-3 py-2 space-y-2">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">EC2 Name</label>
          <input
            type="text"
            value={localData.ec2Name}
            onChange={(e) => updateData({ ec2Name: e.target.value })}
            className="bg-transparent text-xs outline-none border-b border-border focus:border-primary"
            placeholder="my-instance"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">Target Group Name</label>
          <input
            type="text"
            value={localData.targetGroupName}
            onChange={(e) => updateData({ targetGroupName: e.target.value })}
            className="bg-transparent text-xs outline-none border-b border-border focus:border-primary"
...
AWSEC2Node.displayName = 'AWSEC2Node';
