...
export const AWSALBNode = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'Application Load Balancer',
    albUrl: (data as any).albUrl || '',
    ...
    <div className={`rounded-lg border-2 bg-card shadow-lg min-w-[240px]`} style={{ borderColor: `hsl(var(--${color}))` }}>
      ...
      <div className="border-t border-border px-3 py-2 space-y-2">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">ALB URL</label>
          <input
            type="text"
            value={localData.albUrl}
            onChange={(e) => updateData({ albUrl: e.target.value })}
...
AWSALBNode.displayName = 'AWSALBNode';
