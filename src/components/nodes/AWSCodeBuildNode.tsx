...
export const AWSCodeBuildNode = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'CodeBuild Project',
    repoName: (data as any).repoName || '',
    target: (data as any).target || '',
  }));
...
AWSCodeBuildNode.displayName = 'AWSCodeBuildNode';
