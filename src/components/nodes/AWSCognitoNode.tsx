...
export const AWSCognitoNode = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'Cognito User Pool',
    userPoolId: (data as any).userPoolId || '',
    hostedUiUrl: (data as any).hostedUiUrl || '',
    ...
    <div className={`rounded-lg border-2 bg-card shadow-lg min-w-[240px]`} style={{ borderColor: `hsl(var(--${color}))` }}>
      ...
      <div className="border-t border-border px-3 py-2 space-y-2">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">UserPool ID</label>
          <input
            type="text"
            value={localData.userPoolId}
            onChange={(e) => updateData({ userPoolId: e.target.value })}
            className="bg-transparent text-xs outline-none border-b border-border focus:border-primary"
            placeholder="us-east-1_XXXXXXXXX"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">Hosted UI URL</label>
          <input
            type="text"
            value={localData.hostedUiUrl}
            onChange={(e) => updateData({ hostedUiUrl: e.target.value })}
            className="bg-transparent text-xs outline-none border-b border-border focus:border-primary"
            placeholder="https://..."
          />
        </div>
...
AWSCognitoNode.displayName = 'AWSCognitoNode';
