...
export const AWSLambdaNode = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'Lambda Function',
    lambdaName: (data as any).lambdaName || '',
    region: (data as any).region || 'us-east-1',
    functionVersion: (data as any).functionVersion || '$LATEST',
  }));
    ...
    <div className={`rounded-lg border-2 bg-card shadow-lg min-w-[240px]`} style={{ borderColor: `hsl(var(--${color}))` }}>
      ...
      <div className="border-t border-border px-3 py-2 space-y-2">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">Lambda Name</label>
          <input
            type="text"
            value={localData.lambdaName}
            onChange={(e) => updateData({ lambdaName: e.target.value })}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-muted-foreground">Function Version</label>
          <input
            type="text"
            value={localData.functionVersion}
            onChange={(e) => updateData({ functionVersion: e.target.value })}
            className="bg-transparent text-xs outline-none border-b border-border focus:border-primary"
            placeholder="$LATEST"
          />
        </div>
