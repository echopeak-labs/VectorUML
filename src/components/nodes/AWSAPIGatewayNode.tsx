...
export const AWSAPIGatewayNode = memo(({ data, type, id }: NodeProps) => {
  const Icon = getNodeIcon(type as any);
  const color = getNodeColor(type as any);

  const [localData, setLocalData] = useState<any>(() => ({
    name: (data as any).name || 'API Gateway',
    apiName: (data as any).apiName || '',
    stage: (data as any).stage || 'prod',
  }));
...
AWSAPIGatewayNode.displayName = 'AWSAPIGatewayNode';
